...
class Application(config: Config, apiConfig: APIConfig, pdfService: PDFService) extends StrictLogging {
  ...
  val routing = new EndpointsWrapper(pdfRoutes.routes).routing
